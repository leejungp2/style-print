...
데모 영상에서 직접 보여줄 화면은 다음 순서로 잡으면 됩니다.

- UI Facet Mixer 첫 화면에서 1단계, 2단계, 3단계 탭 구조를 보여줍니다.
- Reference Assets 영역에 디자인 레퍼런스 2-3장을 업로드하는 장면을 보여줍니다.
- Extracted Facets 영역에서 색상 팔레트, 타이포그래피, 레이아웃, 간격, 컴포넌트 스타일이 추출되는 장면을 보여줍니다.
- Recommended Recipe를 선택하거나 facet별 source를 수동으로 지정하는 장면을 보여줍니다.
- Coherence Score, conflict, repair plan을 확인하는 장면을 보여줍니다.
- Generate UI 이후 preview, code, audit/provenance 탭을 순서대로 보여줍니다.
